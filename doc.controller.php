...
use App\models\ACC_docs;
use App\models\ACC_DocItems;
use App\models\ACC_DocCheques;
use App\models\WFM_FlowRows;

use Core\dataReader;
use Core\Response;
use Core\phpExcelReader;
...
function selectDocs() {
	...
	if (!empty($_GET["query"]) && !InputValidation::validate($_GET["query"], InputValidation::Pattern_FaEnAlphaNum, false)) {
		echo dataReader::getJsonData(array(), 0, $_GET["callback"]);
		die();
	}
	if (!empty($_GET["fields"]) && !InputValidation::validate($_GET["fields"], InputValidation::Pattern_FaEnAlphaNum, false)) {
		echo dataReader::getJsonData(array(), 0, $_GET["callback"]);
		die();
	}
	
	$where = " sd.CycleID=:cid ";
	$whereParam = array(":cid" => (int)$_SESSION["accounting"]["CycleID"]);
...
function saveDoc() {
	...
	$obj->CycleID = (int)$_SESSION["accounting"]["CycleID"];
